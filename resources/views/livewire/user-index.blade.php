...
<div class="w-full overflow-x-hidden flex flex-col">
    <main class="w-full flex-grow p-6">
        ...
        <span class="text-sm">Manajemen User Aplikasi</span>
                ...
                <table class="min-w-full bg-white">
                    <thead class="bg-sky-700 text-white">
                        <tr>
                            ...
                            <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Username</th>
                    ...
                    <tbody class="text-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="text-center py-3 px-4">{{$item->username}}</td>
                            {{-- <td class="text-center py-3 px-4">{{$item->email}}</td> --}}
                            <td class="text-center py-3 px-4">{{$item->roles->first()->name}}</td>
                            <td class="text-left py-3 px-4 flex justify-end space-x-2">
                                ...
                                <x-atom.button-table-only-faicon icon="fas fa-key" 
                                    warna="amber" class="px-2 py-1 float-right" 
                                    wire:click="$emit('swalAndaYakin','FixResetPasswordUser','{{$item->id}}','anda akan me-reset password menjadi `password`')"/>
                                <x-atom.button-table-only-faicon icon="fas fa-trash" 
                                    warna="red" class="px-2 py-1 float-right" 
                                    wire:click="$emit('swalToDeleted','FixHapusUser','{{$item->id}}')"/>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
